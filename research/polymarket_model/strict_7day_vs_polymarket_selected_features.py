...
This curated variant keeps only the features that showed positive or near-positive
utility in the full leave-one-feature-out ablation against Polymarket.
"""
...
def run_walk_forward_test(
    ...
    if requested_feature_cols is None:
        requested_feature_cols = list(SELECTED_MODEL_FEATURES)
    feature_cols = [c for c in requested_feature_cols if c in binance_df.columns]
    print(
        f"Using {len(feature_cols)}/{len(requested_feature_cols)} selected features. "
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="research/polymarket_model/strict_7day_vs_pm_selected_features_results",
