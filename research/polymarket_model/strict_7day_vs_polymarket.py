...
def fit_sigmoid_calibrator(x_prob: np.ndarray, y: np.ndarray) -> LogisticRegression | None:
    if len(np.unique(y)) < 2:
        return None
    x_prob = np.clip(x_prob, 1e-6, 1 - 1e-6)
    logit_x = np.log(x_prob / (1.0 - x_prob))
    clf = LogisticRegression(max_iter=1000, solver="lbfgs", C=1e5)
    clf.fit(logit_x.reshape(-1, 1), y)
    return clf


def predict_sigmoid(calibrator: LogisticRegression | None, x_prob: np.ndarray) -> np.ndarray:
    if calibrator is None:
        return np.clip(x_prob, 1e-6, 1 - 1e-6)
    x_prob = np.clip(x_prob, 1e-6, 1 - 1e-6)
    logit_x = np.log(x_prob / (1.0 - x_prob))
    pred = calibrator.predict_proba(logit_x.reshape(-1, 1))[:, 1]
...
def tune_shrinkage_lambda(
    ...
    base_gap_pp = float(np.abs(np.mean(p_calib) - np.mean(y_calib)) * 100.0)
    ...
    for lam in [0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00]:
        ...
        gap_pp = float(np.abs(np.mean(p) - np.mean(y_calib)) * 100.0)
...
def build_binance_df(raw_rows: List[List]) -> pd.DataFrame:
    ...
    # Filter out incomplete trailing candle where the full 5-minute window hasn't completed
    max_open_time = df["open_time"].max()
    candle["candle_end"] = candle["candle_5m_start"] + pd.Timedelta(minutes=5)
    candle = candle[candle["candle_end"] <= max_open_time + pd.Timedelta(seconds=1)].copy()
...
def load_polymarket_1s(polymarket_csv: Path) -> pd.DataFrame:
    ...
    for slug, g in df.groupby("slug", sort=False):
        g = g.sort_values("timestamp").drop_duplicates("timestamp", keep="last").copy()
        ...
        for col in ["slug", "start_time", "label", "implied_prob"]:
            u[col] = u[col].ffill()
        u = u.dropna(subset=["slug", "start_time", "label", "implied_prob"]).copy()
...
def compute_metrics(y_true: np.ndarray, y_prob: np.ndarray) -> Dict[str, float]:
    ...
    return {
        ...
        "calibration_gap_pp": float(abs(mean_p - mean_y) * 100.0),
        "ece_10bin_pp": float(compute_ece(y_true, y_prob, n_bins=10) * 100.0),
        "mean_pred": mean_p,
        "mean_actual": mean_y,
    }
...
def run_walk_forward_test(
    ...
    for model_name in ["7day_logistic_baseline", "7day_logistic_enhanced", "polymarket"]:
        ...
        w = model_data["test_rows"] / model_data["test_rows"].sum()
        metrics = {
            "model": model_name,
            "n_splits": model_data["split_idx"].nunique(),
            "test_rows_total": int(model_data["test_rows"].sum()),
            "accuracy_mean": model_data["accuracy"].mean(),
            "accuracy_weighted": float(np.sum(model_data["accuracy"] * w)),
            "accuracy_std": model_data["accuracy"].std(),
            "roc_auc_mean": model_data["roc_auc"].mean(),
            "roc_auc_weighted": float(np.sum(model_data["roc_auc"] * w)),
            "roc_auc_std": model_data["roc_auc"].std(),
            "log_loss_mean": model_data["log_loss"].mean(),
            "log_loss_weighted": float(np.sum(model_data["log_loss"] * w)),
            "log_loss_std": model_data["log_loss"].std(),
            "brier_mean": model_data["brier"].mean(),
            "brier_weighted": float(np.sum(model_data["brier"] * w)),
            "brier_std": model_data["brier"].std(),
            "calibration_gap_pp_mean": model_data["calibration_gap_pp"].mean(),
            "calibration_gap_pp_weighted": float(np.sum(model_data["calibration_gap_pp"] * w)),
            "calibration_gap_pp_std": model_data["calibration_gap_pp"].std(),
            "ece_10bin_pp_mean": model_data["ece_10bin_pp"].mean(),
            "ece_10bin_pp_weighted": float(np.sum(model_data["ece_10bin_pp"] * w)),
            "ece_10bin_pp_std": model_data["ece_10bin_pp"].std(),
        }
...
def main() -> None:
    ...
    print("=" * 60)
    metrics = ["accuracy", "roc_auc", "log_loss", "brier", "calibration_gap_pp", "ece_10bin_pp"]
