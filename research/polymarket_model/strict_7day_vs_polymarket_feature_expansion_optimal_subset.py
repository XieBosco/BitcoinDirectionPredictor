...
def _extract_overall_row(overall_df: pd.DataFrame, model_name: str) -> pd.Series:
    model_rows = overall_df[overall_df["model"] == model_name]
    if model_rows.empty:
        raise ValueError(f"Model {model_name} not found in overall metrics")
    return model_rows.iloc[0]


def metric_tuple(row: pd.Series) -> Tuple[float, float, float, float]:
    return (
        float(row["log_loss_mean"]),
        float(row["brier_mean"]),
        -float(row["roc_auc_mean"]),
        float(row["calibration_gap_pp_mean"]),
    )
...
def evaluate_feature_subset(
    ...
    return run_walk_forward_test(
        ...
        requested_feature_cols=feature_subset,
...
def greedy_subset_search(
    ...
    """Greedy backward elimination that stops once a subset beats the full model."""
    current_subset = list(FULL_FEATURE_SET)
    benchmark_overall, _, _ = evaluate_feature_subset(
        binance_df,
        polymarket_df,
        current_subset,
        training_days=training_days,
    ...
    benchmark_row = _extract_overall_row(benchmark_overall, "7day_logistic_baseline")
    benchmark_score = metric_tuple(benchmark_row)

    history_rows = []
    best_full_subset = list(current_subset)
    best_full_score = benchmark_score
    iteration = 0
    ...
    while True:
        ...
        for dropped in current_subset:
            candidate_subset = [c for c in current_subset if c != dropped]
            if not candidate_subset:
                continue

            quick_overall, _, _ = evaluate_feature_subset(
                binance_df,
                polymarket_df,
                candidate_subset,
                training_days=training_days,
            ...
            quick_row = _extract_overall_row(quick_overall, "7day_logistic_baseline")
            candidate_rows.append(
                {
                    "iteration": iteration,
                    "dropped_feature": dropped,
                    "subset_features": "+".join(candidate_subset),
                    "subset_size": len(candidate_subset),
                    "quick_log_loss_mean": float(quick_row["log_loss_mean"]),
                    "quick_brier_mean": float(quick_row["brier_mean"]),
                    "quick_roc_auc_mean": float(quick_row["roc_auc_mean"]),
                    "quick_calibration_gap_pp_mean": float(quick_row["calibration_gap_pp_mean"]),
                }
            )
        ...
        candidate_df = candidate_df.sort_values(
            ["quick_log_loss_mean", "quick_brier_mean", "quick_roc_auc_mean", "quick_calibration_gap_pp_mean"],
            ascending=[True, True, False, True],
        ).reset_index(drop=True)

        candidate = candidate_df.iloc[0]
        proposed_subset = candidate["subset_features"].split("+")

        proposed_overall, _, _ = evaluate_feature_subset(
            binance_df,
            polymarket_df,
            proposed_subset,
            training_days=training_days,
        ...
        proposed_row = _extract_overall_row(proposed_overall, "7day_logistic_baseline")
        proposed_score = metric_tuple(proposed_row)

        history_rows.append(
            {
                "iteration": iteration,
                "dropped_feature": candidate["dropped_feature"],
                "subset_features": candidate["subset_features"],
                "subset_size": int(candidate["subset_size"]),
                "quick_log_loss_mean": float(candidate["quick_log_loss_mean"]),
                "quick_brier_mean": float(candidate["quick_brier_mean"]),
                "quick_roc_auc_mean": float(candidate["quick_roc_auc_mean"]),
                "quick_calibration_gap_pp_mean": float(candidate["quick_calibration_gap_pp_mean"]),
                "full_log_loss_mean": float(proposed_row["log_loss_mean"]),
                "full_brier_mean": float(proposed_row["brier_mean"]),
                "full_roc_auc_mean": float(proposed_row["roc_auc_mean"]),
                "full_calibration_gap_pp_mean": float(proposed_row["calibration_gap_pp_mean"]),
                "full_score": proposed_score,
                "accepted": bool(proposed_score < best_full_score),
            }
        )

        if proposed_score < best_full_score:
            current_subset = proposed_subset
            best_full_subset = list(proposed_subset)
            best_full_score = proposed_score
            if best_full_score < benchmark_score:
                break
        else:
            break

    history_df = pd.DataFrame(history_rows)
    return best_full_subset, history_df, benchmark_overall
...
def run_walk_forward_test(
    ...
    if requested_feature_cols is None:
        requested_feature_cols = list(FULL_FEATURE_SET)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="research/polymarket_model/strict_7day_vs_pm_feature_expansion_optimal_subset_results",
    ...
    best_subset, subset_history_df, benchmark_overall_df = greedy_subset_search(
        bn,
        pm,
        training_days=7,
    ...
    print(f"Best subset found: {best_subset}")

    print("\nRunning final walk-forward validation (7-day training windows)...")
    overall_df, splits_df, row_level_df = run_walk_forward_test(
        ...
        test_hours=args.test_hours,
        step_hours=args.step_hours,
        requested_feature_cols=best_subset,
    )
    ...
    subset_history_df.to_csv(out_dir / "subset_search_history.csv", index=False)
    benchmark_overall_df.to_csv(out_dir / "benchmark_full_feature_comparison.csv", index=False)
    ...
    print("\n" + "=" * 60)
    print("SUBSET SEARCH SUMMARY")
    print("=" * 60)
    print(f"Best subset: {best_subset}")
    print(f"Benchmark comparison saved to: {out_dir / 'benchmark_full_feature_comparison.csv'}")
    print(f"Search history saved to: {out_dir / 'subset_search_history.csv'}")
